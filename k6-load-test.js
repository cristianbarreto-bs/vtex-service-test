...
function textSummary(data, testOptions) {
  const indent = testOptions.indent || ''
  ...
  summary += `${indent}Request Duration (avg): ${data.metrics.http_req_duration.values.avg.toFixed(
    2
  )}ms\n`
  summary += `${indent}Request Duration (p95): ${data.metrics.http_req_duration.values[
    'p(95)'
  ].toFixed(2)}ms\n`
  summary += `${indent}Request Duration (p99): ${data.metrics.http_req_duration.values[
    'p(99)'
  ].toFixed(2)}ms\n\n`

  // Product Info metrics
  summary += `${indent}Product Info Endpoint:\n`
  summary += `${indent}  Duration (avg): ${data.metrics.product_info_duration.values.avg.toFixed(
    2
  )}ms\n`
  summary += `${indent}  Duration (p95): ${data.metrics.product_info_duration.values[
    'p(95)'
  ].toFixed(2)}ms\n`
  summary += `${indent}  Duration (p99): ${data.metrics.product_info_duration.values[
    'p(99)'
  ].toFixed(2)}ms\n\n`

  // Products by category metrics
  summary += `${indent}Products by category Endpoint:\n`
  summary += `${indent}  Duration (avg): ${data.metrics.products_by_category_info_duration.values.avg.toFixed(
    2
  )}ms\n`
  summary += `${indent}  Duration (p95): ${data.metrics.products_by_category_info_duration.values[
    'p(95)'
  ].toFixed(2)}ms\n`
  summary += `${indent}  Duration (p99): ${data.metrics.products_by_category_info_duration.values[
    'p(99)'
  ].toFixed(2)}ms\n\n`
